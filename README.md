...
Docker Compose can run the app and Postgres locally. The image tag defaults to personal-context-protocol:0.1.8; run `npm run docker:up` to generate `.env`, start Postgres, initialize the app, and print the first-login token when needed. See [Docker Compose deployment](docs/deployment-docker-compose.md).
