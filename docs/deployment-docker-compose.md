...
The default Compose image is `personal-context-protocol:0.1.3`. When `VERSION` changes, set `PCP_VERSION` to the new value before building so each release gets a distinct image tag.
...
|---|---|---|
| `PCP_VERSION` | `0.1.3` | App image tag and Docker build version |
